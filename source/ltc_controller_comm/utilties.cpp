...
bool CompareFileNameStart(const string& file_name, const string& prefix) {
    ...
    for (std::string::const_iterator b = prefix.begin(); b != prefix.end(); ++a, ++b) {
        if (FIX_CHAR(*a) != FIX_CHAR(*b)) { return false; }
    }
    return true;
}
...
path GetPathFromRegistry(const wstring& key_name, const wstring& key_value_name) {
    ...
    result       = RegQueryValueExW(key, key_value_name.c_str(), 0, nullptr,
                              reinterpret_cast<BYTE*>(&path[0]), &size);
    RegCloseKey(key);
...
string ToUtf8(const wstring& utf16) {
    ...
    WideCharToMultiByte(CP_UTF8, 0, utf16.c_str(), narrow<int>(utf16.size()), &utf8[0], utf8_length,
                        0, 0);
    return utf8;
}
...
wstring ToUtf16(const string& utf8) {
    auto utf16_length =
            MultiByteToWideChar(CP_UTF8, 0, utf8.c_str(), narrow<int>(utf8.size()), nullptr, 0);
    wstring utf16(utf16_length, '\0');
    MultiByteToWideChar(CP_UTF8, 0, utf8.c_str(), narrow<int>(utf8.size()), &utf16[0],
                        utf16_length);
    return utf16;
}
...
vector<string> SplitString(const string& str, const char* delimiters, bool keep_empty_fields) {
    ...
    do {
        ...
        if (keep_empty_fields || field.size() > 0) { strs.push_back(field); }
...
void SwapBytesUint32(uint8_t* values, uint32_t num_values) {
    if ((num_values % sizeof(uint32_t)) != 0) { throw logic_error("bad size for uint8 array"); }
