...
class Packet {
   public:
    ...
    static constexpr uint32_t MAC_ADDRESS      = 0x10;

    static constexpr uint32_t FILE_TRANSFER = 0x50;
    static constexpr uint32_t TPP_STRING    = 0x5A;
...
class SocKit : public IDataReceive, public ICollect {
   public:
    ...
    SocKit(LccControllerInfo info) : ip_address(FormatIpAddress(info.id)) {}
    ~SocKit(){};
    ...
    uint32_t ReadMemory(uint32_t memory_address) { return Read(Packet::MEM_READ, memory_address); }
    ...
    void WriteMemoryBlock(uint32_t memory_address, const uint32_t values[], uint32_t num_values) {
    ...
    int ReadRegisterBlockDummy(uint32_t register_address, uint32_t values[], uint32_t num_values) {
        return ReadBlock(Packet::REG_READ_BLOCK | Packet::DUMMY_BIT, register_address, values,
                         num_values);
    }
    ...
    void WriteRegisterBlockDummy(uint32_t       register_address,
                                 const uint32_t values[],
                                 uint32_t       num_values) {
        WriteBlock(Packet::REG_WRITE_BLOCK | Packet::DUMMY_BIT, register_address, values,
                   num_values);
    }
    ...
    int ReadMemoryBlockDummy(uint32_t memory_address, uint32_t values[], uint32_t num_values) {
        return ReadBlock(Packet::MEM_READ_BLOCK | Packet::DUMMY_BIT, memory_address, values,
                         num_values);
    }
    ...
    void WriteMemoryBlockDummy(uint32_t       memory_address,
                               const uint32_t values[],
                               uint32_t       num_values) {
        WriteBlock(Packet::MEM_WRITE_BLOCK | Packet::DUMMY_BIT, memory_address, values, num_values);
    }

    void Shutdown();

   private:
    void   SendPacket(Packet packet, tcp::socket& socket);
    ...
    static string                FormatIpAddress(uint32_t ip_address);
    std::unique_ptr<tcp::socket> MakeSocket();
