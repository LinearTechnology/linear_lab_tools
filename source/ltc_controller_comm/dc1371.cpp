...
int Dc1371::GetNumControllers(int max_controllers) {
    ASSERT_POSITIVE(max_controllers);
    ASSERT_NOT_LARGER(max_controllers, MAX_CONTROLLERS);
...
void Dc1371::DataStartCollect(int total_samples, Trigger trigger) {
    ...
    ASSERT_NOT_SMALLER(total_samples, KILOSAMPLES);
    ASSERT_NOT_LARGER(total_samples, MAX_TOTAL_SAMPLES);
...
int Dc1371::ReadBytes(uint8_t data[], int total_bytes) {
    ...
    ASSERT_NOT_LARGER(total_bytes, 2 * MAX_TOTAL_SAMPLES);
