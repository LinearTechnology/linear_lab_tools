...
TcpClientSocket::TcpClientSocket(uint32_t ip, uint32_t port) : pimpl(std::make_unique<Impl>()) {
    std::stringstream ss;
    ss << (ip >> 24) << '.' << ((ip >> 16) & 0xFF) << '.' << ((ip >> 8) & 0xFF) << '.'
       << (ip & 0xFF);
...
void TcpClientSocket::init(const char* url_or_ip, int port) {
    ...
    if (result != 0) { throw_message_and_code("WSAStartup failed", result); }
...
void TcpClientSocket::send(const uint8_t* bytes, int num_to_write) {
    ...
    if (result == SOCKET_ERROR) { throw_message_and_code("send failed", WSAGetLastError()); }
}
...
std::vector<uint8_t> TcpClientSocket::receive(int num_to_read) {
    ...
    size_t offset = 0;
    while (num_to_read > 0) {
        auto result = recv(pimpl->native_socket, reinterpret_cast<char*>(bytes.data() + offset),
                           num_to_read, 0);
        if (result > 0) {
            num_to_read -= result;
            offset += result;
