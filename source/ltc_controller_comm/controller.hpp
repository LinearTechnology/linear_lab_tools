...
class Controller {
   public:
    ...
    static LccControllerInfo MakeControllerInfo(Type          type,
   ...
   protected:
    ...
    static int DataRead(R& reader, bool swap_bytes, T* values, int num_values) {
        ...
        if (swap_bytes) { SwapBytes(values, num_values); }
        return num_read;
    }
    ...
    static int DataWrite(W& writer, bool swap_bytes, T* values, int num_values) {
        if (swap_bytes) { SwapBytes(values, num_values); }
        int num_written =
                writer.WriteBytes(reinterpret_cast<uint8_t*>(values), num_values * sizeof(T));
