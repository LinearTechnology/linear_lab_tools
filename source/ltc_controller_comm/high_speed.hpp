...
class HighSpeed : public IClose,
                  public IDataEndian,
                  public IDataReceive,
                  public IDataSend,
                  public ISpi {
   public:
    enum class BitMode : uint8_t { MPSSE = FT_BITMODE_MPSSE, FIFO = FT_BITMODE_SYNC_FIFO };

    enum class SpiMode { MODE_0 = 0, MODE_2 = 2 };
    ...
    string GetSerialNumber() override {
        return serial_number_a.substr(0, serial_number_a.size() - 1);
    }
    ...
    // Sets MPSSE SCK divider (default 0) frequency is F / (2 * (1 + divider)) where F is 60 or
    // 12MHz
    ...
    int DataSend(uint8_t data[], int total_bytes) override { return WriteBytes(data, total_bytes); }
   ...
   private:
    ...
    void OpenIfNeeded();
    int  Write(FT_HANDLE handle, uint8_t* values, int num_values, bool allow_partial_write = false);
    int  ReadBytes(uint8_t data[], int total_bytes);
    int  WriteBytes(uint8_t data[], int total_bytes);
    int  Read(FT_HANDLE handle, uint8_t* values, int num_values, bool allow_partial_read = false);
