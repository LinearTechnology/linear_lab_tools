...
#define ENUM_DECLARATION                                                                       \
    ENUM_START                                                                                 \
    ENUM(BAD_NEGATIVE, -1), ENUM(OK, 0), ENUM(GOT_ACK, 1), ENUM(GOT_NAK, 2), ENUM(ABORTED, 3), \
            ENUM(BAD_FOUR, 4), ENUM(BAD_FIVE, 5), ENUM(BAD_SIX, 6), ENUM(BAD_SEVEN, 7),        \
            ENUM(PHASE_ERROR, 8), ENUM(BAD_SIGNATURE, 9), ENUM(BAD_PASSWORD, 10),              \
            ENUM(ILLEGAL_COMMAND, 11), ENUM(GENERIC_ERROR, 12), ENUM(FPGA_LOAD_ERROR, 13),     \
            ENUM(FPGA_ID_ERROR, 14), ENUM(FPGA_PLL_ERROR, 15), ENUM(COLLECT_ERROR, 16),        \
            ENUM(BAD_TOO_LARGE, 17), ENUM_END
...
class Dc1371 : public ICollect,
   ...
   private:
    ...
    struct CommandHeader {
        ...
        CommandHeader(Opcode opcode)
                : signature(QUERY_SIGNATURE), opcode(opcode), byte_param(0), word_param(0) {
    ...
    struct Command {
        ...
        Command(Opcode opcode) : header(opcode) { memset(data, 0, COMMAND_DATA_SIZE); }
    ...
    class CommandFile {
       ...
       public:
        CommandFile(char drive_letter) {
            ...
            file = CreateFileA(file_name, GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING,
                               FILE_FLAG_NO_BUFFERING | FILE_FLAG_WRITE_THROUGH, nullptr);
            if (file == INVALID_HANDLE_VALUE) {
                throw HardwareError("Error opening DC1371A command interface.");
            }
        }
        CommandFile(CommandFile&& other) : file(other.file) { other.file = INVALID_HANDLE_VALUE; }
        ~CommandFile() { Close(); }
        void Close() {
            if (file != INVALID_HANDLE_VALUE) {
    ...
    class BlockFile {
       ...
       public:
        ...
        BlockFile(BlockFile&& other) : file(other.file) { other.file = INVALID_HANDLE_VALUE; }
        ~BlockFile() { Close(); }
        void Close() {
            if (file != INVALID_HANDLE_VALUE) {
                CloseHandle(file);
    ...
    struct FpgaLoadInfo {
        ...
        FpgaLoadInfo(path file_path, uint8_t load_id, int num_blocks, char drive_letter)
                : data(MAX_BLOCKS * BLOCK_SIZE),
                  file_path(file_path),
                  load_id(load_id),
                  offset(0),
                  num_blocks(num_blocks),
                  command_file(drive_letter),
                  block_file(drive_letter) {}
    ...
    void    SpiBufferSendOrTranceive(Command& command,
                                     int&     offset,
                                     uint8_t  send_values[],
                                     int      num_values,
                                     bool     is_send);
    void    SpiBufferReceive(Command& command, int& offset, int num_values);
    uint8_t GetCommandResult(Command&     command,
                             CommandFile& command_file,
                             uint8_t*     data     = nullptr,
                             int          num_data = 0);
    void    CheckCommandResult(Command&     command,
                               CommandFile& command_file,
                               uint8_t*     data     = nullptr,
                               int          num_data = 0);
