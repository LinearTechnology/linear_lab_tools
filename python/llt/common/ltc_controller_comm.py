...
def _ctype_from_string(type_string):
    if type_string == "Bytes":
        ...
        raise errs.ValueError("Invalid type string")
...
class ControllerInfo(ct.Structure):
    ...
    int type;
    char description[consts.MAX_DESCRIPTION_SIZE];
    char serial_number[consts.MAX_SERIAL_NUMBER_SIZE];
    unsigned long id;
    """

    _fields_ = [
        ("_type", ct.c_int),
        ("_description", ct.c_char * consts.DESCRIPTION_BUFFER_SIZE),
        ("_serial_number", ct.c_char * consts.SERIAL_NUMBER_BUFFER_SIZE),
        ("_id", ct.c_ulong)
    ]
    ...
    def get_serial_number(self):
        return self._serial_number[:consts.SERIAL_NUMBER_BUFFER_SIZE]
...
def list_controllers(controller_type):
    ...
    if _dll.LccGetControllerList(ct.c_int(controller_type), controller_info_list,
                                 num_controllers) != 0:
        raise errs.HardwareError("Could not get device info list, or no device found")
...
class Controller(object):
    ...
    def __init__(self, controller_info):
        ...
        self._c_error_buffer = ct.create_string_buffer(consts.ERROR_BUFFER_SIZE)
        self._c_array = None
    ...
    def _call(self, func_name, *args):
        ...
        if error_code != 0:
            self._dll.LccGetErrorInfo(self._handle, self._c_error_buffer, consts.ERROR_BUFFER_SIZE)
            if error_code == -1:
    ...
    def get_serial_number(self):
        ...
        c_serial_number = ct.create_string_buffer(consts.SERIAL_NUMBER_BUFFER_SIZE)
        self._call('GetSerialNumber', c_serial_number, consts.SERIAL_NUMBER_BUFFER_SIZE)
        return c_serial_number.value

    def get_description(self):
        """Return the current controller's description."""
        c_description = ct.create_string_buffer(consts.DESCRIPTION_BUFFER_SIZE)
        self._call('GetDescription', c_description, consts.DESCRIPTION_BUFFER_SIZE)
