...
PID_KP_OFFSET = 0x60 #Not used in this demo, but defined
PID_KI_OFFSET = 0x70#Not used in this demo, but defined
PID_KD_OFFSET = 0x80#Not used in this demo, but defined
PULSE_LOW_OFFSET = 0x90#Not used in this demo, but defined
PULSE_HIGH_OFFSET = 0xA0#Not used in this demo, but defined
PULSE_VAL_OFFSET = 0xB0#Not used in this demo, but defined
...
for i in range(0, 5):
    print ('run # %d ' % i)
    client.reg_write(TUNING_WORD_OFFSET, 500 + i * 100) # Sweep NCO!!!
    sleep(0.1)
    print("Starting control system...\n");
    client.reg_write(START_OFFSET, CHANNEL | 0x00000001);
    client.reg_write(START_OFFSET, CHANNEL | 0);

 
    sleep(1); #sleep for a tenth of a second
    ...
    for k in range(0, NUM_SAMPLES): # Fix sign extension
        if(data[k] >= 524288):
            data[k] -= 1048576
        pythondata[k] = data[k]
    amplitude = max(pythondata) - min(pythondata)
